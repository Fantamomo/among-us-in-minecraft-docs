---
title: Commands
description: Overview of all plugin commands including /amongusadmin, /amongus, and /impostermsg with their subcommands, permissions, and usage.
---
...
See [Area Configuration](../create-an-area).
...
:::caution[Selecting]
Selecting players via `@a`, `@p` etc. works like in vanilla Minecraft.
However, some predicates such as `gamemode=...` are not supported.
All predicates are supported when using `@s`.
Other selectors only support `sort` and `limit` — additional predicates will be accepted but have no effect.

This limitation exists because some players may no longer be online.
:::

:::note[Custom Selector]
This plugin introduces a custom predicate `game=<code>` to target players within a specific game.
It works both in the console and on the client side.
Note that the client is unaware of this predicate, so the command will appear highlighted in red.
...
When used in the console, all currently running games will be suggested as autocomplete options.

You can also select all players that are **not** in a game by using `@a[game=!CODE]`.
:::